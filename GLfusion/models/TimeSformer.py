...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, n, device):
        seq = torch.arange(n * 4, device = device)
...
def attn(q, k, v, mask = None):
    sim = einsum('b v i d, b v j d -> b v i j', q, k)
    ...
    out = einsum('b v i j, b v j d -> b v i d', attn, v)
    return out

def attn_(q, k, v, mask = None):
...
class Attention(nn.Module):
    ...
    def forward(self, x, einops_from, einops_to, mask = None, cls_mask = None, rot_emb = None, **einops_dims):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b v n (h d) -> (b h) v n d', h = h), (q, k, v))
        ...
        cls_k, cls_v = map(lambda t: repeat(t, 'b v n d -> (b r) (v n) d', r = r), (cls_k, cls_v))

        k_ = torch.cat((cls_k, k_), dim = 1)
        v_ = torch.cat((cls_v, v_), dim = 1)

        # attention
        out = attn_(q_, k_, v_, mask = mask)

        # merge back time or space
        out = rearrange(out, f'{einops_to} -> {einops_from}', **einops_dims)

        # concat back the cls token
        out = torch.cat((cls_out, out), dim = 2)

        # merge back the heads
        out = rearrange(out, '(b h) v n d -> b v n (h d)', h = h)
...
class TimeSformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.cls_token = nn.Parameter(torch.randn(4, dim))
    ...
    def forward(self, video, mask = None):
        b, v, f, h, w, *_, device, p = *video.shape, video.device, self.patch_size
        ...
        video = rearrange(video, 'b v f (h p1) (w p2) -> b v (f h w) (p1 p2)', p1 = p, p2 = p)
        tokens = self.to_patch_embedding(video)

        # add cls token

        cls_token = repeat(self.cls_token, 'v d -> b v n d', b = b, n = 1)
        x =  torch.cat((cls_token, tokens), dim = 2)
        ...
        for (time_attn, spatial_attn, ff) in self.layers:
            x = time_attn(x, 'b v (f n) d', '(b n) (v f) d', n = n, f = f, v = v, mask = frame_mask, cls_mask = cls_attn_mask, rot_emb = frame_pos_emb) + x
            x = spatial_attn(x, 'b v (f n) d', '(b f) (v n) d', n = n, f = f, v = v, cls_mask = cls_attn_mask, rot_emb = image_pos_emb) + x
            x = ff(x) + x

        cls_token = x[:, :, 0]
        return self.to_out(cls_token.reshape(b, -1))
...
if __name__ == "__main__":
    ...
    input_tensor = torch.autograd.Variable(torch.rand(2, 4, 64, 112, 112))
